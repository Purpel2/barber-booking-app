...
type Barber = { id: string; name: string; imageUrl: string };
type PortfolioItem = { id: string; barberId: string; imageUrl: string };

// Karta elementu portfolio
    ...
    return (
        <div className="break-inside-avoid mb-6 group cursor-pointer">
            <div
                ...
                className="relative overflow-hidden bg-surface-container-low rounded-xl shadow-lg border border-outline-variant/5 aspect-auto"
            >
...
/**
 * PortfolioClient to komponent klienta, który renderuje stronę portfolio barbera.
 * Umożliwia filtrowanie portfolio według barbera, aby użytkownicy mogli zobaczyć prace konkretnego barbera.
 * Pobiera dane barberów i portfolio jako propsy i renderuje je w responsywnym układzie.
        ...
        <div className="min-h-screen">
            ...
            <section className="px-4 sm:px-8 lg:px-20 mb-16 max-w-7xl mx-auto overflow-hidden">
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4 overflow-x-auto no-scrollbar space-x-4 pb-4 items-center">
                    <label
                        className={`flex items-center gap-4 shrink-0 px-8 h-18 rounded-full cursor-pointer transition-all duration-300 border ${selectedBarber === "all"
